...
int main(void)
{
  ...
  while (1)
  {
	  while((ADCtoCRR(pollADC())/47999.0)*100 > 65);
	  HAL_Delay(450);
	  if((ADCtoCRR(pollADC())/47999.0)*100 > 65)

	  {
		  HAL_Delay(450);
		  if((ADCtoCRR(pollADC())/47999.0)*100 < 40)
		  {
			  sprintf(buffer, "Message Started! \n\r");
			  HAL_Delay(450);
			  print(buffer);
			  char result[4];
			  for(int i = 0; i < 4; i++){
